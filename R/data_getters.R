...
download_res <- function(resId) {
  ...
  utils::download.file(res$url, dataZip)
  if (is_zip(dataZip)){
...
#' @export
get_tif_res <- function(resId, crop_sf=NULL, crs=4326, tifFile=NULL,
                        getSf=FALSE) {
  dataDir <- download_res(resId)
...
#' @export
get_data_res <- function(resId, fileName=NULL, savePath=NULL) {
  dataDir <- download_res(resId)
