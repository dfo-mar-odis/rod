...
test_that("download url", {
  zipFile <- "https://github.com/stoyelq/rod/raw/main/tests/testthat/testdata/testShp.zip"
  dataDir <- rod:::download_url(zipFile)
  testthat::expect_gt(length(list.files(dataDir)), 0)
})

test_that("trigger error on res not found", {
  shpFile <- "https://github.com/stoyelq/rod/raw/main/tests/testthat/testdata/testShp.zip"
...
test_that("gdb getter", {
  gdbFile <- "https://github.com/stoyelq/rod/raw/main/tests/testthat/testdata/testGdb.zip"
  outSf_list<- rod:::get_gdb_res(gdbFile, crs=4326)
  expect_equal(sf::st_crs(outSf_list[[1]])$input, "EPSG:4326")
})

test_that("csv getter", {
  csvFile <- "https://github.com/stoyelq/rod/raw/main/tests/testthat/testdata/test.csv"
  outDir <- rod:::get_data_res(csvFile)
  expect_gt(length(list.files(outDir)), 0)
})

test_that("tif getter", {
  tifFile <- "https://github.com/stoyelq/rod/raw/main/tests/testthat/testdata/testTif.zip"
